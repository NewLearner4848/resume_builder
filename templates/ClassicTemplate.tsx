...
const ResumeSection: React.FC<{ title: string; children: React.ReactNode; className?: string }> = ({ title, children, className = '' }) => (
    <div className={`mt-6 ${className}`}>
        <h2 className="text-xl font-bold text-[--accent-color] border-b-2 border-slate-200 pb-1 mb-3 font-serif">{title}</h2>
        {children}
    </div>
);
...
export const ClassicTemplate: React.FC<TemplateProps> = ({ data, accentColor }) => {
  ...
  return (
    <div className="bg-white p-8 lg:p-12 shadow-2xl rounded-lg border border-slate-200 font-serif" style={{ '--accent-color': accentColor } as React.CSSProperties}>
      <header className="text-center mb-8">
        <h1 className="text-4xl font-bold text-slate-800 tracking-tight">{fullName}</h1>
        <div className="mt-3 flex justify-center items-center flex-wrap gap-x-4 gap-y-2 text-sm">
      ...
      <main>
          ...
          <ResumeSection title="Core Competencies">
            <div className="flex flex-wrap gap-2">
              {skills.map((skill, index) => (
                <span key={index} className="bg-slate-100 text-slate-700 text-sm font-medium px-3 py-1 rounded-full">{skill}</span>
              ))}
            </div>
          </ResumeSection>
        )}
            ...
            {workExperience.map(exp => (
              <div key={exp.id} className="mb-5 last:mb-0">
                <div className="flex justify-between items-baseline">
                  ...
                  <p className="text-sm font-medium text-slate-600">{exp.startDate} - {exp.endDate}</p>
                </div>
                <h4 className="text-md font-semibold text-slate-600 mb-2">{exp.company}</h4>
                <ul className="pl-4 text-sm space-y-1">{renderDescription(exp.description)}</ul>
              </div>
            ))}
          </ResumeSection>
        )}
        ...
        {education && education.length > 0 && (
          <ResumeSection title="Education">
            {education.map(edu => (
              <div key={edu.id} className="flex justify-between items-baseline mb-2 last:mb-0">
                <div>
                  <h3 className="text-lg font-bold text-slate-800">{edu.degree}</h3>
                  <p className="text-md text-slate-600">{edu.school}</p>
                </div>
                <p className="text-sm font-medium text-slate-600">{edu.gradDate}</p>
              </div>
            ))}
          </ResumeSection>
        )}
