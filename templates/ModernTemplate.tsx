    ...
    <div className="w-[210mm] min-h-[297mm] bg-white shadow-2xl rounded-lg border border-slate-200 font-sans flex flex-col md:flex-row" style={{ '--accent-color': accentColor, '--accent-color-light': accentColorLight } as React.CSSProperties}>
      ...
      <aside className="w-full md:w-1/3 bg-slate-50 p-4 sm:p-6 text-slate-800 md:border-r border-slate-200">
        <div className="md:sticky md:top-6">
            <h1 className="text-3xl sm:text-4xl font-bold text-[--accent-color] leading-tight">{fullName}</h1>
            <h2 className="text-md sm:text-lg font-medium text-slate-600 mt-1">{workExperience[0]?.jobTitle || 'Professional'}</h2>
      ...
      {/* Main Content */}
      <main className="w-full md:w-2/3 p-4 sm:p-8">
        <section>
          <h3 className="text-xl sm:text-2xl font-bold text-[--accent-color] border-b-2 border-slate-200 pb-2 mb-4">Profile</h3>
          <p className="text-slate-700 leading-relaxed text-sm sm:text-base">{professionalSummary}</p>
        </section>

        {workExperience && workExperience.length > 0 && (
          <section className="mt-8">
            <h3 className="text-xl sm:text-2xl font-bold text-[--accent-color] border-b-2 border-slate-200 pb-2 mb-4">Experience</h3>
            {workExperience.map(exp => (
              <div key={exp.id} className="mb-6 last:mb-0">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline">
                  <h4 className="text-lg sm:text-xl font-semibold text-slate-800">{exp.jobTitle}</h4>
                ...
                <ul className="text-sm sm:text-base space-y-1">{renderDescription(exp.description)}</ul>
              </div>
            ))}
          </section>
        )}
